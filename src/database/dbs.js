...
//       `);
//  tx.executeSql("DROP TABLE IF EXISTS admin_users;");
...
// export const insertAdminUser = (user, onSuccess) => {
//   db.transaction((tx) => {
//     tx.executeSql(
//       `INSERT INTO admin_users (first_name, age, gender, hobbies, country, dob, bio)
//        VALUES (?, ?, ?, ?, ?, ?, ?)`,
//       [
//         user.first_name,
//         user.age,
//         user.gender,
//         JSON.stringify(user.hobbies), // store as string
//         user.country,
//         user.dob,
//         user.bio,
//       ],
...
// // FETCH
// export const fetchAdminUsers = (callback) => {
//   db.transaction((tx) => {
//     tx.executeSql(
...
//           let u = rows.item(i);
//           u.hobbies = u.hobbies ? JSON.parse(u.hobbies) : [];
//           users.push(u);
//         }
//         callback(users);
//       },
...
export const initAdminUsersTable = () => {
  db.transaction((tx) => {
    tx.executeSql(
      `CREATE TABLE IF NOT EXISTS admin_users (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
...
export const updateAdminUser = (user, onSuccess) => {
  db.transaction((tx) => {
    tx.executeSql(
      `UPDATE admin_users
       SET first_name = ?, age = ?, gender = ?, hobbies = ?, country = ?, dob = ?, bio = ?
       WHERE id = ?`,
      [
        user.first_name,
        user.age,
  ...
  db.transaction(
    (tx) => {
      tx.executeSql(
        ...
        [id],
        (_, result) => {
          console.log("✅ Admin user deleted", result);
          if (onSuccess) onSuccess();
        },
