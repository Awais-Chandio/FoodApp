...
//code above is before today friday  
...
//   name: { fontWeight: "bold", fontSize: 14, flexWrap: "wrap" },
//   subText: { color: "#555", fontSize: 12, marginTop: 2, flexWrap: "wrap" },
//   priceText: { fontSize: 13, fontWeight: "600", marginTop: 2, color: "#FF7000" },
//   offerTag: {
//     position: "absolute",
//     top: 6,
//     left: 6,
...
//   // run every time screen comes back into focus
//   useFocusEffect(
//     useCallback(() => {
//       loadRestaurants();
//     }, [loadRestaurants])
...
//   const getImageSource = (item) => {
//     if (item.image_path) {
//       // http(s) image from DB
//       if (/^https?:\/\//i.test(item.image_path)) {
//         return { uri: item.image_path };
//       }
//       // fallback to bundled key without extension
//       const key = item.image_path.replace(/\.(png|jpg|jpeg)$/i, "");
//       if (restaurantImages[key]) return restaurantImages[key];
//     }
//     // default fallback
...
//above is old manage items code
// import React, { useState, useEffect, useCallback } from "react";
// import {
//   View,
//   Text,
...
//   // decide which image to display (more robust than before)
//   const getImageSource = (item) => {
...
//     if (path) {
//       // If it's a network or local file/content/data URI, return as uri
//       if (/^(https?:|file:|content:|data:)/i.test(path)) {
//         return { uri: path };
//       }

//       // Try key without extension first (e.g. "Westway.png" -> "Westway")
//       const cleaned = path.replace(/\.(png|jpe?g|webp)$/i, "");
//       if (restaurantImages[cleaned]) return restaurantImages[cleaned];

//       // Try the raw value as a key too (in case user saved "1" or "Moonland")
//       if (restaurantImages[path]) return restaurantImages[path];
//     }

//     // fallback to id-based image or default
//     return restaurantImages[item.id] || restaurantImages["1"];
//   };

//   const renderRestaurant = ({ item }) => {
//     // Get first menu item's price (if available). We show price only when a valid price exists.
//     const firstMenu = item?.menu_items?.[0] ?? null;
//     const firstPriceRaw = firstMenu?.price;
//     const firstPrice = (firstPriceRaw !== null && firstPriceRaw !== undefined && firstPriceRaw !== "")
//       ? Number(firstPriceRaw)
//       : null;

//     const shouldShowPrice = firstPrice !== null && Number.isFinite(firstPrice) && firstPrice > 0;
...
//           {/* show price from first menu item (only if it exists and is a positive number) */}
//           {shouldShowPrice && (
//             <Text style={styles.priceText}>Rs. {firstPrice.toFixed(2)}</Text>
//           )}
//         </View>
